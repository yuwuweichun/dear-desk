...
describe('procedural scene model factories', () => {
  it('builds the room shell with a real north opening and four panes', () => {
    ...
    expect(STUDY_ROOM_MODEL_SPEC.ceilingInset).toBe(0.02)
    ...
    expect(runtime.nodes.ceiling.name).toBe('study-room-ceiling-background-blocker')
    expect(runtime.nodes.ceiling.position.y).toBeCloseTo(12.955)
    expect(runtime.nodes.ceiling.material).toBeInstanceOf(THREE.MeshBasicMaterial)
    expect((runtime.nodes.ceiling.material as THREE.MeshBasicMaterial).color.getHexString()).toBe('d4c6b4')
    expect(runtime.nodes.ceilingTray.name).toBe('study-room-ceiling-tray')
    expect(runtime.nodes.ceilingTray.count).toBe(4)
    expect(STUDY_ROOM_MODEL_SPEC.ceilingTray.drop).toBeGreaterThan(0)
    expect(STUDY_ROOM_MODEL_SPEC.ceilingTray.soffitWidth).toBe(2.4)
    expect((runtime.nodes.ceilingTray.material as THREE.MeshStandardMaterial).color.getHexString()).toBe('e6dac9')
    expect(runtime.nodes.ceilingCrown.name).toBe('study-room-ceiling-crown')
    ...
    expect(runtime.nodes.windowReveal.material).toBe(runtime.nodes.westWall.material)
    expect(runtime.nodes.windowBackdrop.name).toBe('study-room-window-outdoor-backdrop')
    expect(runtime.nodes.northWindow.getObjectByName('study-room-window-sill')).toBeUndefined()
    expect(runtime.nodes.northWindow.getObjectByName('study-room-window-apron')).toBeUndefined()
    ...
    const wallAlbedo = (runtime.nodes.westWall.material as THREE.MeshStandardMaterial).map as THREE.DataTexture
    expect(wallAlbedo.image.width).toBe(512)
    expect(Array.from((wallAlbedo.image.data as Uint8Array).slice(0, 3))).toEqual([150, 146, 121])
    const windowFrame = runtime.nodes.northWindow.getObjectByName('study-room-window-frame-left') as THREE.Mesh
    expect(windowFrame.position.x).toBe(-STUDY_ROOM_MODEL_SPEC.window.frameInset)
    expect((windowFrame.material as THREE.MeshStandardMaterial).color.getHexString()).toBe('b98b69')
    expect((windowFrame.material as THREE.MeshStandardMaterial).map).toBe((runtime.nodes.floor.material as THREE.MeshStandardMaterial).map)
    expect((runtime.nodes.ceiling.material as THREE.MeshBasicMaterial).map).toBeNull()
    ...
    expect(room.userData.resourceMetrics.drawCalls).toBeLessThanOrEqual(30)
  })
  ...
  it('maps Candy Cloud from one five-face cube-net atlas', () => {
    ...
    const northMaterial = runtime.nodes.northWall.material as THREE.MeshStandardMaterial
