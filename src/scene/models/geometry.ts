...
export function createContinuousOpenSpreadGeometry(
  width: number,
  depth: number,
) {
  ...
  const heightAt = (x: number, z: number) => {
    const distanceFromFold = Math.abs(x)
    const acrossPage = Math.min(1, distanceFromFold / halfWidth)
    const foldValley = 0.032 * Math.exp(-Math.pow(distanceFromFold / 0.12, 2))
    const rootShoulder = 0.055 * (
      1 - Math.exp(-Math.pow(distanceFromFold / 0.22, 2))
    ) * Math.exp(-distanceFromFold / 1.05)
    const broadCrown = 0.028 * Math.sin(Math.PI * acrossPage)
    const headTailLift = 0.012 * Math.pow(Math.abs(z) / halfDepth, 3) *
      THREE.MathUtils.smoothstep(distanceFromFold, 0.08, 0.3)
    return rootShoulder + broadCrown + headTailLift - foldValley
  }

  for (let row = 0; row <= depthSegments; row += 1) {
    const v = row / depthSegments
    const z = -halfDepth + v * depth
  ...
  const appendEndCap = (z: number, reverse: boolean) => {
    const start = positions.length / 3
    for (let column = 0; column <= widthSegments; column += 1) {
      const u = column / widthSegments
      const x = -halfWidth + u * width
      positions.push(x, heightAt(x, z), z, x, -0.045, z)
      uvs.push(u, 1, u, 0)
    }
    for (let column = 0; column < widthSegments; column += 1) {
      const top = start + column * 2
      const bottom = top + 1
      const nextTop = top + 2
  ...
  appendEndCap(-halfDepth, false)
  appendEndCap(halfDepth, true)
